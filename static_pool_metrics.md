# 🏦 Static Pool Data
| id | Feature | Description | Derivation | Type | Source | Pseudocode Added |
|----|----------|--------------|-------------|------|--------|-------------------|
...
| 6  | token0_address             | Address of first token in pair                                                                    | Token address                                                                        | String           | CSV, Graph             | ✅ |
| 7  | token1_address             | Address of second token in pair                                                                   | Token address                                                                        | String           | CSV, Graph             | ✅ |
| 8  | token0_name                | Name of the first token in pair                                                                   | Fetch via Graph, RPC, or CSV (preferably from token contract's name() or metadata)   | String           | CSV, Graph, RPC        | ✅ |
| 9  | token1_name                | Name of the second token in pair                                                                  | Fetch via Graph, RPC, or CSV (preferably from token contract's name() or metadata)   | String           | CSV, Graph, RPC        | ✅ |
| 10 | token0_symbol              | Symbol of the first token in pair                                                                 | Fetch via Graph, RPC, or CSV (preferably from token contract's symbol() or metadata) | String           | CSV, Graph, RPC        | ✅ |
| 11 | token1_symbol              | Symbol of the second token in pair                                                                | Fetch via Graph, RPC, or CSV (preferably from token contract's symbol() or metadata) | String           | CSV, Graph, RPC        | ✅ |
| 12 | token0_pool_amount         | Amount of pools on DEX with first token                                                           | Count fetched pools                                                                  | Integer          | Graph                  | ✅ |
| 13 | token1_pool_amount         | Amount of pools on DEX with second token                                                          | Count fetched pools                                                                  | Integer          | Graph                  | ✅ |
| 14 | token0_total_supply        | Total supply defined in first token contract                                                     | Use batched eth_call of totalSupply() at your target block. Multicall/fallback logic | Float            | RPC                    | ✅ |
| 15 | token1_total_supply        | Total supply defined in second token contract                                                    | Use batched eth_call of totalSupply() at your target block. Multicall/fallback logic | Float            | RPC                    | ✅ |
| 16 | token0_age_minutes         | Time from first token contract creation to DEX listing                                            | Get pool creation timestamp from The Graph, get token contract creation timestamp from Etherscan, calculate age in minutes  | Integer (minutes)| Etherscan, Graph       | ✅ |
| 17 | token1_age_minutes         | Time from second token contract creation to DEX listing                                           | Get pool creation timestamp from The Graph, get token contract creation timestamp from Etherscan, calculate age in minutes | Integer (minutes)| Etherscan, Graph       | ✅ |
...
| 20 | token0_ownership_renounced | Is ownership of the first token contract renounced?                                               | Use eth_call of owner(), via multicall/direct call                                   | Boolean          | RPC                    | ✅ |
